...
13. [Catatan Integrasi Hexos](#hexos-integration)
...
### Device Binding & Multi-Account

**machineId TIDAK terikat ke hardware.** Di Linux, setiap proses qodercli
generate machineId baru dari random string → UUID. Di Windows/Mac ada fallback
ke disk serial + MAC address, tapi ini bisa di-override.

```
# Dari log:
Using random string: wvTY4lKndWZGjMVamaA1 as machineId
Generated uuid: 77765459-346c-4b2d-ae64-576c5a476a2d
# Proses berikutnya:
Using random string: EEBe7LuxeNhrXTK7ZjKM as machineId
Generated uuid: 45454265-374c-452d-b865-4e6c6872582d
```

**machineToken terikat ke user account**, bukan device. Implikasi:

- Multi-account bisa dengan swap `~/.qoder/.auth/` folder
- Tidak ada device fingerprint yang mencegah multi-login
- machineToken tidak expire (valid selamanya sampai logout)
- Quota per-account, reset monthly (`nextResetAt` timestamp)
- `isQuotaExceeded` flag tapi server masih terima request (soft limit)
- `ip_banned_error` ada di binary — server bisa ban IP
...
| 2026-04-23 | Device binding analysis — no hardware lock |

---

## 13. Catatan Integrasi Hexos <a name="hexos-integration"></a>

Dokumen ini adalah bagian dari project [hexos](https://github.com/kadangkesel/hexos)
— multi-provider AI API proxy. Qoder **belum** menjadi provider yang didukung hexos.
Saat ini hexos mendukung 3 provider: Service (Tencent), Cline, dan Kiro (AWS CodeWhisperer).

### Status Integrasi Qoder → Hexos

| Komponen | Status | Catatan |
|----------|:------:|---------|
| Auth algorithm | ✅ Fully reversed | Signature + Bearer COSY, semua crypto primitives diketahui |
| API endpoints | ✅ Mapped | 25+ endpoints, inference SSE verified |
| Model catalog | ✅ Complete | 9 public + 15+ hidden models |
| Format translation | ⚠️ Partial | Response SSE sudah OpenAI-compatible, request body perlu encrypt |
| Body encryption | ⚠️ Blocker | AES-128-CBC custom encoding belum fully reversed |
| Multi-account | ✅ Feasible | Tidak ada device lock, auth file swappable |
| Browser automation | ❌ Not started | Perlu Camoufox script untuk Google OAuth login |

### Apa yang Dibutuhkan untuk Menambah Qoder sebagai Provider Hexos

1. **`src/proxy/qoder.ts`** — Format translator (minimal, response sudah OpenAI-compatible)
2. **`src/proxy/qoder-auth.ts`** — Token generator (implement Bearer COSY algorithm)
3. **`src/automation/qoder_login.py`** — Camoufox script untuk batch login via Google OAuth
4. **`src/config/providers.ts`** — Tambah Qoder provider definition
5. **`src/config/models.ts`** — Tambah 9+ Qoder models ke catalog

### Tantangan Utama

**Request body encryption** adalah blocker terbesar. Opsi:

| Approach | Effort | Reliability |
|----------|:------:|:-----------:|
| Reverse custom encoding dari binary | Hard | High |
| Relay via qodercli binary (spawn process) | Easy | Medium |
| MITM proxy (swap headers, let client encrypt) | Medium | High |
| Extract encryption code dari IDE JS | Medium | High |

Rekomendasi: Extract dari IDE JS (`sharedProcessMain.js`) karena kode sudah
readable dan algorithm sudah diketahui. File tersedia di:
`/home/ubuntu/qoder_files/EXTRACTED_AUTH_CODE.txt`

### Arsitektur Proxy yang Diusulkan

```
Client Request (OpenAI format)
  → hexos /v1/chat/completions
    → Qoder provider handler
      → Generate Bearer COSY token (from extracted algorithm)
      → Set X-Model-Key header (model mapping)
      → Encrypt request body (AES-128-CBC)
      → POST api2.qoder.sh/algo/api/v2/service/pro/sse/agent_chat_generation
      → Parse SSE response (already OpenAI-compatible)
      → Stream back to client
```

Response tidak perlu di-translate karena Qoder SSE sudah mengembalikan
`chat.completion.chunk` format yang identik dengan OpenAI.
